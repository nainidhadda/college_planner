...
if finance_list:
    for idx, plan in enumerate(finance_list):
        with st.expander(f"{plan['type']}: {plan['name']}"):
            # Remove: st.write(plan)  # This shows raw JSON data
            # Instead, show formatted information:
            if plan['type'] == "Savings Goal":
                target = plan.get('target', 0)
                saved = plan.get('saved', 0)
                pct = (saved / target * 100) if target else 0
                st.markdown(f'<span style="color:#fff;">**Progress:** {pct:.1f}% (₹{saved:,.0f} / ₹{target:,.0f})</span>', unsafe_allow_html=True)
                st.progress(min(pct / 100, 1.0) if target else 0)
            elif plan['type'] == "SIP":
                st.markdown(f'<span style="color:#fff;">**Monthly Investment:** ₹{plan.get("amount", 0):,.0f}</span>', unsafe_allow_html=True)
                st.markdown(f'<span style="color:#fff;">**Expected Return:** {plan.get("rate", 0)}%</span>', unsafe_allow_html=True)
                st.markdown(f'<span style="color:#fff;">**Duration:** {plan.get("years", 0)} years</span>', unsafe_allow_html=True)
                # SIP line chart
                ...
                st.markdown(f'<span style="color:#fff;">Projected SIP Value after {plan["years"]} years: ₹{growth[-1]:,.2f}</span>', unsafe_allow_html=True)
            elif plan['type'] == "Monthly Budget":
                st.markdown(f'<span style="color:#fff;">**Monthly Income:** ₹{plan.get("income", 0):,.0f}</span>', unsafe_allow_html=True)
                st.markdown(f'<span style="color:#fff;">**Monthly Expenses:** ₹{plan.get("expenses", 0):,.0f}</span>', unsafe_allow_html=True)
                # Monthly Budget pie chart for categories
                categories = plan.get('categories', {})
                if not categories:
                    st.markdown('<span style="color:#fff;">Add budget categories below to see allocation chart.</span>', unsafe_allow_html=True)
                else:
                    cat_df = pd.DataFrame({'Category': list(categories.keys()), 'Amount': list(categories.values())})
                    st.markdown('<span style="color:#fff;">**Budget Allocation:**</span>', unsafe_allow_html=True)
                    st.plotly_chart({
                        "data": [{
                ...
                with st.form(f"cat_form_{idx}", clear_on_submit=True):
                    ...
                    if categories:
                        ...
                        if del_cat != "-" and st.form_submit_button("Delete Selected Category"):
                            ...
                            st.rerun()
            elif plan['type'] == "Stock Experiment":
                st.markdown(f'<span style="color:#fff;">**Stock:** {plan.get("stock", "")}</span>', unsafe_allow_html=True)
                st.markdown(f'<span style="color:#fff;">**Amount Invested:** ₹{plan.get("invested", 0):,.0f}</span>', unsafe_allow_html=True)
                st.markdown(f'<span style="color:#fff;">**Current Value:** ₹{plan.get("result", 0):,.0f}</span>', unsafe_allow_html=True)
...
st.progress(min(done_count / total_goals, 1.0) if total_goals > 0 else 0)
...
for idx, subject in enumerate(subjects):
    ...
    with st.expander(f"Subject: {subject}"):
        ...
        st.markdown(f'<span style="color:#fff;">**Progress:** {progress_pct:.1f}% ({done_tasks}/{total_tasks} tasks done)</span>', unsafe_allow_html=True)
        st.progress(min(progress_pct / 100, 1.0) if total_tasks > 0 else 0)
        ...
        if total_tasks > 0:
            ...
            st.markdown('<span style="color:#fff;">**Task Status Distribution:**</span>', unsafe_allow_html=True)
            st.plotly_chart(
                {
                    "data": [
                        {
